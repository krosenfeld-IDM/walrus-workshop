...
# # (optional) to visualize layout quickly
# for ax in [ax00, ax01, ax10, ax11, ax_big]:
...
simulation = np.squeeze(trajectory['input_fields'][0, error.step:error.step+cfg.walrus.n_steps_input+1, ..., field_id][-1]).numpy()
vmax = np.max(np.abs(simulation))
im11 = ax11.imshow(simulation, cmap='gray', vmin=-vmax, vmax=vmax)
ax11.set_title("Tracer")
ax11.text(0.01,-0.08, f"Timestep: {error.step+cfg.walrus.n_steps_input}", transform=ax11.transAxes, ha="left", va="top")
...
ax21.text(0.01,-0.08, f"Timestep: {error.step+cfg.walrus.n_steps_input}", transform=ax21.transAxes, ha="left", va="top")
